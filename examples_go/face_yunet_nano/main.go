// Face detection + recognition using YuNet-Nano (yunet_n_320_320.onnx).
//
// YuNet-Nano is a lighter detector variant optimized for speed.
// This example uses oscar_selfies.jpg (multi-face) by default.
//
// Build & run:
//
...
func main() {
	detectorPath := flag.String("detector", "yunet_n_320_320.onnx", "path to YuNet-Nano ONNX model")
	recognizerPath := flag.String("recognizer", "w600k_mbf.onnx", "path to ArcFace ONNX model")
	imagePath := flag.String("image", "oscar_selfies.jpg", "path to test image")
	outputPath := flag.String("output", "output.jpg", "path to save result image")
	conf := flag.Float64("conf", 0.5, "confidence threshold")
	...
	if count > 0 {
		...
		for i, f := range faces {
			...
			for k := 0; k < 5; k++ {
				fmt.Printf(" (%.1f,%.1f)", f.landmarks[k*2], f.landmarks[k*2+1])
			}
			fmt.Println()
